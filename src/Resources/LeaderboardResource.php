...
class LeaderboardResource extends BaseResource
{
    ...
    public function getHubSeasonRanking(string $hubId, int $season, int $offset = 0, int $limit = 20): EntityRanking
    {
        $request = new GetHubSeasonRankingRequest($hubId, $season, $offset, $limit);
        $response = $this->connector->send($request);
    ...
    public function getPlayerRanking(string $leaderboardId, string $playerId): Ranking
    {
        $request = new GetPlayerLeaderboardRankingRequest($leaderboardId, $playerId);
        $response = $this->connector->send($request);
